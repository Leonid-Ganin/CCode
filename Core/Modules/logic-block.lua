...
M.new = function(name, scroll, group, index, event, params, comment)
    ...
    if event then y = y + 24 end
    table.insert(group.blocks, index, display.newGroup())
    group.blocks[index].data = {event = event, comment = comment, name = name, params = params}
    ...
    group.scrollHeight = group.scrollHeight + group.blocks[index].block.height
